...
int DivPlatformSysMDExt::dispatch(DivCommand c) {
  if (c.chan<2) {
    return DivPlatformSysMD::dispatch(c);
  }
  if (c.chan>5) {
    c.chan-=3;
    return DivPlatformSysMD::dispatch(c);
...
void DivPlatformSysMDExt::muteChannel(int ch, bool mute) {
  if (ch<2) {
    DivPlatformSysMD::muteChannel(ch,mute);
    return;
  }
  if (ch>5) {
    DivPlatformSysMD::muteChannel(ch-3,mute);
    return;
  }
  isOpMuted[ch-2]=mute;
...
void DivPlatformSysMDExt::tick(bool sysTick) {
  if (extMode) {
...
void DivPlatformSysMDExt::forceIns() {
  for (int i=0; i<6; i++) {
    for (int j=0; j<4; j++) {
...
void* DivPlatformSysMDExt::getChanState(int ch) {
  if (ch>=6) return &chan[ch-3];
  if (ch>=2) return &opChan[ch-2];
  return &chan[ch];
}

DivMacroInt* DivPlatformSysMDExt::getChanMacroInt(int ch) {
  if (ch>=6) return &chan[ch-3].std;
  if (ch>=2) return NULL; // currently not implemented
  return &chan[ch].std;
}

DivDispatchOscBuffer* DivPlatformSysMDExt::getOscBuffer(int ch) {
  if (ch>=6) return oscBuf[ch-3];
  if (ch<3) return oscBuf[ch];
  return NULL;
}

void DivPlatformSysMDExt::reset() {
  DivPlatformSysMD::reset();

  for (int i=0; i<4; i++) {
    opChan[i]=DivPlatformSysMDExt::OpChannel();
    opChan[i].vol=127;
  }
...
bool DivPlatformSysMDExt::keyOffAffectsArp(int ch) {
  return (ch>8);
}

bool DivPlatformSysMDExt::keyOffAffectsPorta(int ch) {
  return (ch>8);
}

void DivPlatformSysMDExt::notifyInsChange(int ins) {
  DivPlatformSysMD::notifyInsChange(ins);
  for (int i=0; i<4; i++) {
    if (opChan[i].ins==ins) {
      opChan[i].insChanged=true;
    }
...
int DivPlatformSysMDExt::getPortaFloor(int ch) {
  return (ch>8)?12:0;
}

int DivPlatformSysMDExt::init(DivEngine* parent, int channels, int sugRate, const DivConfig& flags) {
  DivPlatformSysMD::init(parent,channels,sugRate,flags);
  for (int i=0; i<4; i++) {
    isOpMuted[i]=false;
  }
  extSys=true;
...
void DivPlatformSysMDExt::quit() {
  DivPlatformSysMD::quit();
}

DivPlatformSysMDExt::~DivPlatformSysMDExt() {
}
