...
void DivPlatformSysMD::processDAC() {
  if (softPCM) {
...
void DivPlatformSysMD::acquire_nuked(short* bufL, short* bufR, size_t start, size_t len) {
  static short o[2];
  static int os[2];
  ...
  for (size_t h=start; h<start+len; h++) {
    ...
    for (int i=0; i<6; i++) {
      if (!writes.empty() && --delay<0) {
        ...
        if (w.addrOrVal) {
          OPF2_Write(&fm,0x1+((w.addr>>8)<<1),w.val);
          //printf("write: %x = %.2x\n",w.addr,w.val);
          lastBusy=0;
          ...
          if (fm.write_busy==0) {
            ...
            OPF2_Write(&fm,0x0+((w.addr>>8)<<1),w.addr);
            w.addrOrVal=true;
          }
        }
      }
      
      OPF2_Clock(&fm,o); os[0]+=o[0]; os[1]+=o[1];
      //OPF2_Write(&fm,0,0);
      if (i==5) {
        if (fm.dacen) {
          if (softPCM) {
...
//void DivPlatformSysMD::acquire_ymfm(short* bufL, short* bufR, size_t start, size_t len) {
//  static int os[2];
//
...
void DivPlatformSysMD::acquire(short* bufL, short* bufR, size_t start, size_t len) {
  /*if (useYMFM) {
...
void DivPlatformSysMD::tick(bool sysTick) {
...
void DivPlatformSysMD::muteChannel(int ch, bool mute) {
  isMuted[ch]=mute;
  if (ch>6) return;
  if (ch<6) {
    for (int j=0; j<4; j++) {
...
int DivPlatformSysMD::dispatch(DivCommand c) {
  switch (c.cmd) {
...
void DivPlatformSysMD::forceIns() {
  for (int i=0; i<6; i++) {
    for (int j=0; j<4; j++) {
      unsigned short baseAddr=chanOffs[i]|opOffs[j];
      DivInstrumentFM::Operator& op=chan[i].state.op[j];
...
void DivPlatformSysMD::toggleRegisterDump(bool enable) {
  DivDispatch::toggleRegisterDump(enable);
}

void* DivPlatformSysMD::getChanState(int ch) {
  return &chan[ch];
}

DivMacroInt* DivPlatformSysMD::getChanMacroInt(int ch) {
  return &chan[ch].std;
}

DivDispatchOscBuffer* DivPlatformSysMD::getOscBuffer(int ch) {
  return oscBuf[ch];
}

unsigned char* DivPlatformSysMD::getRegisterPool() {
  return regPool;
}

int DivPlatformSysMD::getRegisterPoolSize() {
  return 512;
}

void DivPlatformSysMD::reset() {
  ...
  OPF2_Reset(&fm);
  OPF2_SetChipType(ladder?ym5173_mode_ym2612:0);
  if (dumpWrites) {
    addWrite(0xffffffff,0);
  }
  for (int i=0; i<10; i++) {
    chan[i]=DivPlatformSysMD::Channel();
...
bool DivPlatformSysMD::isStereo() {
  return true;
}

bool DivPlatformSysMD::keyOffAffectsArp(int ch) {
  return (ch>5);
}

bool DivPlatformSysMD::keyOffAffectsPorta(int ch) {
  return (ch>5);
}

void DivPlatformSysMD::notifyInsChange(int ins) {
  for (int i=0; i<10; i++) {
    if (chan[i].ins==ins) {
...
void DivPlatformSysMD::notifyInsDeletion(void* ins) {
}

void DivPlatformSysMD::poke(unsigned int addr, unsigned short val) {
  immWrite(addr,val);
}

void DivPlatformSysMD::poke(std::vector<DivRegWrite>& wlist) {
  for (DivRegWrite& i: wlist) immWrite(i.addr,i.val);
}

int DivPlatformSysMD::getPortaFloor(int ch) {
  return (ch>5)?12:0;
}

//void DivPlatformSysMD::setYMFM(bool use) {
//  useYMFM=use;
//}

void DivPlatformSysMD::setSoftPCM(bool value) {
  softPCM=value;
}

void DivPlatformSysMD::setFlags(const DivConfig& flags) {
...
int DivPlatformSysMD::init(DivEngine* p, int channels, int sugRate, const DivConfig& flags) {
  parent=p;
  dumpWrites=false;
  ladder=false;
...
DivPlatformSysMD::~DivPlatformSysMD() {
}
