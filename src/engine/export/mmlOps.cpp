...
String sanitizeName(const String& name) {
    ...
    for (char c : name) {
        if (isalnum(c) || c == '_' || c == '-') {
            ...
            sanitized += '_'; // Replace illegal characters with underscore
        }
    }
    return sanitized;
}

SafeWriter* DivEngine::saveMMLSNESAMK(int amkVersion, bool hasDefaultHeader, const String& exportDir) {
    SafeWriter* w = new SafeWriter;
    w->init();
    w->writeText(fmt::sprintf("#amk %d\n\n", amkVersion));
    w->writeText(fmt::sprintf("; Furnace AddMusicK Export\n; Generated by Furnace %s (%d)\n\n",DIV_VERSION,DIV_ENGINE_VERSION));
    w->writeText("#SPC\n{");
    w->writeText(fmt::sprintf("\n\t#title \"%s\"\n", song.name));
    w->writeText(fmt::sprintf("\t#game \"%s\"\n", song.category));
    w->writeText("\t#comment \"Furnace AddMusicK Output\"\n");
    w->writeText(fmt::sprintf("\t#author \"%s\"\n", song.author));
    w->writeText("}\n\n");

    // Define and create a new directory for our samples
    String relativeDir = sanitizeName(song.name + " samples");
    String samplesOutputDir = exportDir + DIR_SEPARATOR + relativeDir;
    logD("Sample output dir: %s", samplesOutputDir.c_str());
    if (!dirExists(samplesOutputDir.c_str())) {
      if (!makeDir(samplesOutputDir.c_str())) {
        logW("Could not create sample directory: %s", samplesOutputDir.c_str());
      }
    }

    w->writeText(fmt::sprintf("#path \"%s\"\n\n", relativeDir));
    String sampleList = "#samples\n{\n";
    ...
    for (int i = 0; i < song.sampleLen; i++) {
        ...
        if (sample && !sample->name.empty()) {
            const String& name = sample->name;
            sampleNames.push_back(name);
            sampleList += fmt::sprintf("\t\"%s.brr\"\n", name.c_str());

            // Make sure it's BRR-encoded
            if (sample->depth != DIV_SAMPLE_DEPTH_BRR) {
                int sampleCount = sample->loop ? sample->loopEnd : sample->samples;
                bool ok = sample->initInternal(DIV_SAMPLE_DEPTH_BRR, sampleCount);
                if (!ok) {
                  logW("Sample \"%s\" could not be converted to BRR!", name.c_str());
                  continue;
                }

                // Re-encode sample data into BRR
                brrEncode(
                    sample->data16,         // source PCM16
                    sample->dataBRR,        // destination BRR buffer
                    sampleCount,
                    sample->loop ? sample->loopStart : -1,
                    sample->brrEmphasis,
                    sample->brrNoFilter
                );
            }

            // Now we can write the BRR file safely
            String fullPath = samplesOutputDir + DIR_SEPARATOR + name + ".brr";
            if (!sample->saveRaw(fullPath.c_str())) {
              logW("Could not write sample file: %s", fullPath.c_str());
            }
        }
    }
    ...
    int baseInstrumentIndex = hasDefaultHeader ? 30 : 0;

    // Helper for formatting one line
    auto formatInstAMKLine = [](const String& name, const DivInstrumentSNES& snes, double inPitch, const String& instName, int index) {
        String line;

        // Sample name, quoted
        String quotedName = "\"" + name + ".brr\"";
        line += fmt::sprintf("\t%-24s", quotedName.c_str());  // pad to 24 characters left-aligned

        // Compute ADSR bytes or zeroes if gain is used
        uint8_t adsr1 = snes.useEnv ? (0x80 | ((snes.d & 0x07) << 4) | (snes.a & 0x0F)) : 0x00;
        ...
        line += fmt::sprintf("; @%02d - %s", index, instName.c_str());

        // Append ADSR or Gain description
        if (snes.useEnv) {
            line += fmt::sprintf(" (ADSR: %X %X %X %X)", snes.a, snes.d, snes.s, snes.r);
        } else {
            line += fmt::sprintf(" (Gain: %02X)", snes.gain);
        }
    ...
    for (int i = 0; i < song.insLen; i++) {
      DivInstrument* inst = getIns(i);
      if (!inst) continue;

      const DivInstrumentSNES& snes = inst->snes;
      DivSample* instSamp = getSample(inst->amiga.getSample(0));
