...
void DivEngine::findSongLength(int loopOrder, int loopRow, double fadeoutLen, int& rowsForFadeout, bool& hasFFxx, std::vector<int>& orders, int& length, int& loopTick) {
  if (curSubSong!=NULL) {
    curSubSong->findLength(loopOrder,loopRow,fadeoutLen,rowsForFadeout,hasFFxx,orders,song.grooves,length,loopTick,chans,song.jumpTreatment,song.ignoreJumpAtEnd);
  }
}
