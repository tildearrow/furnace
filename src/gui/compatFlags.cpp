  ...
  if (ImGui::Begin("Compatibility Flags",&compatFlagsOpen,globalWinFlags,_("Compatibility Flags"))) {
    ...
    if (ImGui::BeginTabBar("settingsTab")) {
      if (ImGui::BeginTabItem(_("DefleMask"))) {
        ...
        if (InvCheckbox(_("Don't apply Game Boy envelope on note-less instrument change"),&e->song.gbInsAffectsEnvelope)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("if this is on, an instrument change will not affect the envelope."));
        }
        if (ImGui::Checkbox(_("Ignore DAC mode change outside of intended channel in ExtCh mode"),&e->song.ignoreDACModeOutsideIntendedChannel)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("if this is on, 17xx has no effect on the operator channels in YM2612."));
        }
        if (ImGui::Checkbox(_("E1xy/E2xy also take priority over slide stops"),&e->song.e1e2AlsoTakePriority)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("does this make any sense by now?"));
        }
        if (ImGui::Checkbox(_("E1xy/E2xy stop when repeating the same note"),&e->song.e1e2StopOnSameNote)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("ugh, if only this wasn't a thing..."));
        }
        if (ImGui::Checkbox(_("SN76489 duty macro always resets phase"),&e->song.snDutyReset)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("when enabled, duty macro will always reset phase, even if its value hasn't changed."));
        }
        if (InvCheckbox(_("Broken volume scaling strategy"),&e->song.newVolumeScaling)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("when enabled:\n- log scaling: multiply\n- linear scaling: subtract\nwhen disabled:\n- log scaling: subtract\n- linear scaling: multiply"));
        }
        if (InvCheckbox(_("Don't persist volume macro after it finishes"),&e->song.volMacroLinger)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
        ...
        if (ImGui::Checkbox(_("Broken output volume on instrument change"),&e->song.brokenOutVol)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
        ...
        if (ImGui::Checkbox(_("Broken output volume - Episode 2 (PLEASE KEEP ME DISABLED)"),&e->song.brokenOutVol2)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
        ...
        if (ImGui::Checkbox(_("Treat SN76489 periods under 8 as 1"),&e->song.snNoLowPeriods)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
      ...
      if (ImGui::BeginTabItem(_("Old Furnace"))) {
        if (ImGui::Checkbox(_("Arpeggio inhibits non-porta slides"),&e->song.arpNonPorta)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.5.5"));
        }
        if (ImGui::Checkbox(_("Wack FM algorithm macro"),&e->song.algMacroBehavior)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.5.5"));
        }
        if (ImGui::Checkbox(_("Broken shortcut slides (E1xy/E2xy)"),&e->song.brokenShortcutSlides)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.5.7"));
        }
        if (ImGui::Checkbox(_("Stop portamento on note off"),&e->song.stopPortaOnNoteOff)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre1"));
        }
        if (InvCheckbox(_("Don't allow instrument change during slides"),&e->song.newInsTriggersInPorta)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre1"));
        }
        if (InvCheckbox(_("Don't reset note to base on arpeggio stop"),&e->song.arp0Reset)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre1"));
        }
        if (InvCheckbox(_("ExtCh channel status is not shared among operators"),&e->song.sharedExtStat)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre1"));
        }
        if (InvCheckbox(_("Disable new SegaPCM features (macros and better panning)"),&e->song.newSegaPCM)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre1"));
        }
        if (ImGui::Checkbox(_("Old FM octave boundary behavior"),&e->song.oldOctaveBoundary)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre1"));
        }
        if (ImGui::Checkbox(_("Disable OPN2 DAC volume control"),&e->song.noOPN2Vol)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre1"));
        }
        if (ImGui::Checkbox(_("Broken initial position of portamento after arpeggio"),&e->song.brokenPortaArp)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre1.5"));
        }
        if (ImGui::Checkbox(_("Disable new sample features"),&e->song.disableSampleMacro)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre2"));
        }
        if (ImGui::Checkbox(_("Old arpeggio macro + pitch slide strategy"),&e->song.oldArpStrategy)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre2"));
        }
        if (ImGui::Checkbox(_("Broken portamento during legato"),&e->song.brokenPortaLegato)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre4"));
        }
        if (ImGui::Checkbox(_("Broken macros in some FM chips after note off"),&e->song.brokenFMOff)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre5"));
        }
        if (ImGui::Checkbox(_("Pre-note does not take effects into consideration"),&e->song.preNoteNoEffect)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6pre9"));
        }
        if (ImGui::Checkbox(_("Disable new NES DPCM features"),&e->song.oldDPCM)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6.1"));
        }
        if (ImGui::Checkbox(_("Legacy technical ALWAYS_SET_VOLUME behavior"),&e->song.oldAlwaysSetVolume)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("behavior changed in 0.6.1\nthis flag will be removed if I find out that none of the songs break after disabling it."));
        }
        if (ImGui::Checkbox(_("Old sample offset effect"),&e->song.oldSampleOffset)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
      ...
      if (ImGui::BeginTabItem(_(".mod import"))) {
        if (ImGui::Checkbox(_("Don't slide on the first tick of a row"),&e->song.noSlidesOnFirstTick)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("simulates ProTracker's behavior of not applying volume/pitch slides on the first tick of a row."));
        }
        if (ImGui::Checkbox(_("Reset arpeggio position on row change"),&e->song.rowResetsArpPos)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
      ...
      if (ImGui::BeginTabItem(_("Other"))) {
        if (ImGui::Checkbox(_("Auto-insert one tick gap between notes"),&e->song.oneTickCut)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
        ...
        if (InvCheckbox(_("Don't reset slides after note off"),&e->song.noteOffResetsSlides)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("when enabled, note off will not reset the channel's slide effect."));
        }
        if (InvCheckbox(_("Don't reset portamento after reaching target"),&e->song.targetResetsSlides)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("when enabled, the slide effect will not be disabled after it reaches its target."));
        }
        if (ImGui::Checkbox(_("Continuous vibrato"),&e->song.continuousVibrato)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("when enabled, vibrato phase/position will not be reset on a new note."));
        }
        if (InvCheckbox(_("Pitch macro is not linear"),&e->song.pitchMacroIsLinear)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("when enabled, the pitch macro of an instrument is in frequency/period space."));
        }
        if (ImGui::Checkbox(_("Reset arpeggio effect position on new note"),&e->song.resetArpPhaseOnNewNote)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(_("when enabled, arpeggio effect (00xy) position is reset on a new note."));
        }
        if (ImGui::Checkbox(_("Volume scaling rounds up"),&e->song.ceilVolumeScaling)) MARK_MODIFIED
        if (ImGui::IsItemHovered()) {
