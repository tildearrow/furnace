      ...
      if (ImGui::BeginTabBar("insEditTab")) {
        ...
        if (ImGui::BeginTabItem("Macros")) {
          ...
          if (settings.macroView==0) { // modern view
            ...
            NORMAL_MACRO(ins->std.arpMacro,arpMacroScroll,arpMacroScroll+24,"arp","Arpeggio",160,ins->std.arpMacro.open,false,NULL,true,&arpMacroScroll,(arpMode?-60:-95),95,0,0,true,1,macroAbsoluteMode,uiColors[GUI_COLOR_MACRO_PITCH],mmlString[1],-95,95,(ins->std.arpMacro.mode?(&macroHoverNote):NULL),true);
            if (dutyMax>0) {
              if (ins->type==DIV_INS_MIKEY) {
