...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const char* label_end = FindRenderedTextEnd(label);
    const ImVec2 label_size = CalcTextSize(label, label_end, (flags&ImGuiButtonFlags_NoHashTextHide)?false:true);
    ...
    if (flags & ImGuiButtonFlags_NoHashTextHide) {
      RenderTextClippedNoHashHide(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, label_end, &label_size, style.ButtonTextAlign, &bb);
    } else {
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    {
        label_end = FindRenderedTextEndNoHashHide(label);
    ...
    if (is_visible)
    {
        const bool highlighted = ((hovered && (held || !(g.IO.ConfigFlags&ImGuiConfigFlags_NoHoverColors))) || (flags & ImGuiSelectableFlags_Highlight));
    ...
    if (is_visible) {
      if(flags & ImGuiSelectableFlags_NoHashTextHide)
      {
          RenderTextClippedNoHashHide(pos, ImVec2(ImMin(pos.x + size.x, window->WorkRect.Max.x), pos.y + size.y), label, label_end, &label_size, style.SelectableTextAlign, &bb);
      }
      else
      {
