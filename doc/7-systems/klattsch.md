...
- `18xx`: **set voicing** (`00` to `FE`). sticky. `FF` returns control to the phoneme.
- `19xx`: **set aspiration** (`00` to `FE`). sticky. `FF` returns control to the phoneme.
- `1Axx`: **set spectral tilt** as a two's-complement signed value. `00` to `7F` is neutral to positive (brighter); `80` to `FE` is negative (darker). sticky. `FF` returns control to the phoneme.
- `1Bxx`: **set glottal effort** (`00` to `FE`). sticky. `FF` returns control to the phoneme.
...
- `1Exx`: **set gain** to `xx`/16. sticky. default is `38` (3.5); `00` resets to default.
- `1Fxx`: **set formant bandwidth scale** to `xx`/64. sticky. `40` is neutral; smaller values narrow the resonators into ringing filters. `00` resets to neutral.
- `20xx`: **set formant shift** to `xx`/64. sticky. `40` is neutral; values above shift every formant up, shrinking the apparent vocal tract. `00` resets to neutral.
